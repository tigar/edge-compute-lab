import { getBotType, isBot } from '../botDetection.js';
 ...
 */
function createMockRequest(userAgent: string | null): MockRequest {
  return {
    headers: {
      get: (name: string): string | null => {
...
describe('botDetection', () => {
  describe('isBot', () => {
    test('should detect Googlebot', () => {
      ...
      expect(isBot(request as unknown as Request)).toBe(true);
    });
    ...
    test('should not detect Chrome as a bot', () => {
      ...
      expect(isBot(request as unknown as Request)).toBe(false);
    });
  ...
  describe('getBotType', () => {
    test('should return bot type for Googlebot', () => {
      const request = createMockRequest('Mozilla/5.0 (compatible; Googlebot/2.1)');
      expect(getBotType(request as unknown as Request)).toBe('googlebot');
    });

    test('should return bot type for Facebook crawler', () => {
      const request = createMockRequest('facebookexternalhit/1.1');
      expect(getBotType(request as unknown as Request)).toBe('facebookexternalhit');
    });
    ...
    test('should return null for human browsers', () => {
      ...
      expect(getBotType(request as unknown as Request)).toBe(null);
    });
